// For creation of gui window
// contains the implemntation of application window frame, gui functions
...
GuiFrame::GuiFrame(const wxString &filepath, const wxString &initialContent)
...
void GuiFrame::BindEventHandlers()
{
    // File operations
    Bind(wxEVT_MENU, &GuiFrame::OnNewFile, this, ID_NewFile);
    Bind(wxEVT_MENU, &GuiFrame::OnNewWindow, this, ID_NewWindow);
    Bind(wxEVT_MENU, &GuiFrame::OnOpenFile, this, ID_OpenFile);
    Bind(wxEVT_MENU, &GuiFrame::OnOpenFolder, this, ID_OpenFolder);

    // Edit operations
    Bind(wxEVT_MENU, &GuiFrame::OnCut, this, ID_Cut);
    Bind(wxEVT_MENU, &GuiFrame::OnCopy, this, ID_Copy);
    Bind(wxEVT_MENU, &GuiFrame::OnPaste, this, ID_Paste);
    Bind(wxEVT_MENU, &GuiFrame::OnUndo, this, wxID_UNDO);
    Bind(wxEVT_MENU, &GuiFrame::OnRedo, this, wxID_REDO);
    Bind(wxEVT_MENU, &GuiFrame::OnWrap, this, ID_Wrap);

    // Find and Replace operations
    Bind(wxEVT_MENU, &GuiFrame::OnFind, this, ID_Find);
    Bind(wxEVT_MENU, &GuiFrame::OnReplace, this, ID_Replace);
    Bind(wxEVT_MENU, &GuiFrame::OnFindNext, this, ID_FindNext);
    Bind(wxEVT_MENU, &GuiFrame::OnFindPrevious, this, ID_FindPrevious);
    Bind(wxEVT_FIND, &GuiFrame::OnFindDialogFind, this);
    Bind(wxEVT_FIND_NEXT, &GuiFrame::OnFindDialogFind, this);
    Bind(wxEVT_FIND_REPLACE, &GuiFrame::OnFindDialogReplace, this);
    Bind(wxEVT_FIND_REPLACE_ALL, &GuiFrame::OnFindDialogReplaceAll, this);
    Bind(wxEVT_FIND_CLOSE, &GuiFrame::OnFindDialogClose, this);

    // Theme bindings
    Bind(wxEVT_MENU, &GuiFrame::OnChangeTheme, this, ID_ThemeDefault);
    Bind(wxEVT_MENU, &GuiFrame::OnChangeTheme, this, ID_ThemeDark);
    Bind(wxEVT_MENU, &GuiFrame::OnChangeTheme, this, ID_ThemeBluishGrey);
    Bind(wxEVT_MENU, &GuiFrame::OnChangeTheme, this, ID_ThemeSolarizedLight);
    Bind(wxEVT_MENU, &GuiFrame::OnChangeTheme, this, ID_ThemeSolarizedDark);
    Bind(wxEVT_MENU, &GuiFrame::OnChangeTheme, this, ID_ThemeMonokai);
    Bind(wxEVT_MENU, &GuiFrame::OnChangeTheme, this, ID_ThemeDracula);
    Bind(wxEVT_MENU, &GuiFrame::OnChangeTheme, this, ID_ThemeGitHubLight);
    Bind(wxEVT_MENU, &GuiFrame::OnChangeTheme, this, ID_ThemeNord);

    // Help Menu bindings
    Bind(wxEVT_MENU, &GuiFrame::onDocumentation, this, ID_Documentation);
    Bind(wxEVT_MENU, &GuiFrame::onAbout, this, wxID_ABOUT);

    // UI and Layout events
    Bind(wxEVT_BUTTON, &GuiFrame::ToggleSidePanel, this, ID_ToggleButton);
    Bind(wxEVT_BUTTON, &GuiFrame::ToggleSearch, this, ID_ShowSearch);
    Bind(wxEVT_TIMER, &GuiFrame::onTimer, this);
    m_treeCtrl->Bind(wxEVT_TREE_ITEM_ACTIVATED, &GuiFrame::OnTreeItemActivated, this);
    m_notebook->Bind(wxEVT_AUINOTEBOOK_PAGE_CLOSE, &GuiFrame::onTabClose, this);
    m_notebook->Bind(wxEVT_AUINOTEBOOK_PAGE_CHANGED, &GuiFrame::OnTabChange, this);
    Bind(wxEVT_BUTTON, &GuiFrame::OnZoomButtonClick, this, m_zoomButton->GetId());

    // Search control events
    m_searchCtrl->Bind(wxEVT_COMMAND_TEXT_ENTER, &GuiFrame::OnSearchCtrl, this);
    m_searchCtrl->Bind(wxEVT_COMMAND_SEARCHCTRL_SEARCH_BTN, &GuiFrame::OnSearchCtrl, this);
    Bind(wxEVT_FIND, &GuiFrame::OnFindDialogFind, this);
    Bind(wxEVT_FIND_NEXT, &GuiFrame::OnFindDialogFind, this);
    Bind(wxEVT_FIND_REPLACE, &GuiFrame::OnFindDialogReplace, this);
    Bind(wxEVT_FIND_REPLACE_ALL, &GuiFrame::OnFindDialogReplaceAll, this);
    ...
    // saveas
    Bind(wxEVT_MENU, &GuiFrame::OnSave, this, ID_Save);
    Bind(wxEVT_MENU, &GuiFrame::OnSaveAs, this, ID_SaveAs);
}
...
void GuiFrame::CreateLayout()
{
    ...
    m_searchCtrl->Bind(wxEVT_COMMAND_TEXT_ENTER, &GuiFrame::OnSearchCtrl, this);
    m_searchCtrl->Bind(wxEVT_COMMAND_SEARCHCTRL_SEARCH_BTN, &GuiFrame::OnSearchCtrl, this);
    replaceButton->Bind(wxEVT_BUTTON, &GuiFrame::OnReplaceSidePanel, this);
    ////////////////////////////////////////////////////////
}

// File operations
void GuiFrame::OnNewFile(wxCommandEvent &event)
{
    HideWelcomePage();
    CreateUntitledTab();
}

void GuiFrame::OnNewWindow(wxCommandEvent &event)
{
    GuiFrame *newFrame = new GuiFrame(wxEmptyString, wxEmptyString);
    newFrame->Show(true);
}
...
void GuiFrame::OnSaveAs(wxCommandEvent &event)
{
    Editor *currentEditor = GetCurrentEditor();
    if (!currentEditor)
    {
...
void GuiFrame::OnOpenFolder(wxCommandEvent &event)
{
...
void GuiFrame::PopulateTreeWithDirs(const wxString &path, wxTreeItemId parentId)
{
    wxDir dir(path);
    if (!dir.IsOpened())
    {
...
wxString GuiFrame::GetItemPath(wxTreeItemId itemId)
{
...
void GuiFrame::SetCurrentLanguage()
{
    Editor *currentEditor = GetCurrentEditor();
...
wxString GuiFrame::ConvertExtension(wxString &fileExtension)
{
...
void GuiFrame::CreateTab(const wxString &filename)
{
...
void GuiFrame::CreateUntitledTab()
{
...
void GuiFrame::onTabClose(wxAuiNotebookEvent &event)
{
...
void GuiFrame::onCloseTabCleanup(wxCommandEvent &event)
{
...
void GuiFrame::OnTabChange(wxAuiNotebookEvent &event)
{
    UpdateTitle();
    SetCurrentLanguage();
    event.Skip();
...
void GuiFrame::UpdateTitle(int count)
{
    if (count == 0)
    {
        SetTitle("CodeLite");
...
void GuiFrame::UpdateTitle(const wxString &filePath)
{
...
void GuiFrame::OnTreeItemActivated(wxTreeEvent &event)
{
...
void GuiFrame::OnZoomButtonClick(wxCommandEvent &event)
{
    Editor *currentEditor = GetCurrentEditor();
...
void GuiFrame::UpdateZoom(int zoom)
{
    int level = zoom;
    m_zoomButton->SetLabel(wxString::Format("Zoom: %d", level));
}

void GuiFrame::ToggleSidePanel(wxCommandEvent &event)
{
...
void GuiFrame::ToggleSearch(wxCommandEvent &event)
{
    if (!m_isSearchEnabled)
    {
        if (m_isSidePanelShown)
...
void GuiFrame::OnCut(wxCommandEvent &event)
{
    Editor *currentEditor = GetCurrentEditor();
    if (currentEditor)
    {
...
void GuiFrame::OnCopy(wxCommandEvent &event)
{
    Editor *currentEditor = GetCurrentEditor();
    if (currentEditor)
    {
...
void GuiFrame::OnPaste(wxCommandEvent &event)
{
    Editor *currentEditor = GetCurrentEditor();
    if (currentEditor)
    {
...
void GuiFrame::OnWrap(wxCommandEvent &event)
{
...
void GuiFrame::CreateFindReplaceDialog()
{
...
void GuiFrame::OnFind(wxCommandEvent &event)
{
    CreateFindReplaceDialog();
}

void GuiFrame::OnReplace(wxCommandEvent &event)
{
    CreateFindReplaceDialog();
}

void GuiFrame::OnFindNext(wxCommandEvent &event)
{
    Editor *currentEditor = GetCurrentEditor();
    if (currentEditor)
    {
...
void GuiFrame::OnFindPrevious(wxCommandEvent &event)
{
    Editor *currentEditor = GetCurrentEditor();
    if (currentEditor)
    {
...
void GuiFrame::OnFindDialogFind(wxFindDialogEvent &event)
{
    Editor *currentEditor = GetCurrentEditor();
    if (!currentEditor)
        return;
...
void GuiFrame::OnFindDialogReplace(wxFindDialogEvent &event)
{
    Editor *currentEditor = GetCurrentEditor();
    if (!currentEditor)
        return;
...
void GuiFrame::OnFindDialogReplaceAll(wxFindDialogEvent &event)
{
    Editor *currentEditor = GetCurrentEditor();
    if (currentEditor)
    {
...
void GuiFrame::OnFindDialogClose(wxFindDialogEvent &event)
{
    m_findReplaceDialog->Destroy();
    m_findReplaceDialog = nullptr;
}

void GuiFrame::OnSearchCtrl(wxCommandEvent &event)
{
...
void GuiFrame::OnReplaceSidePanel(wxCommandEvent &event)
{
    wxString searchString = m_searchCtrl->GetValue();
...
void GuiFrame::OnChangeTheme(wxCommandEvent &event)
{
...
void GuiFrame::ApplyTheme(const Theme &theme)
{
    for (Editor *editor : m_editors)
    {
...
void GuiFrame::onAbout(wxCommandEvent &event)
{
    wxString message =
        "                             CodeLite\n"
        "A simple text editor application with basic features for file manipulation.\n\n"
        "Version - 1.0.0\n\n"
        "Credits\n"
        "Sanat Kumar Lamichanne\n"
        "Shrawan Thapa\n"
        "Sujal Pandit\n"
        "\n\n"
        "With profound passion and genuine delight";

    wxMessageBox(message, "About CodeLite", wxOK | wxICON_INFORMATION, this);
}

void GuiFrame::onDocumentation(wxCommandEvent &event)
{
...
Editor *GuiFrame::GetCurrentEditor()
{
    int currentPage = m_notebook->GetSelection();
